...
if(MSVC)
	...
	set(CMAKE_CXX_FLAGS "/std:c++17 /Wall /W4 /EHsc /MP /bigobj /Zc:strictStrings /Zc:rvalueCast /Gw")

	if(CMAKE_C_COMPILER_ID MATCHES Clang)
		set(TORCH_COMPILER_NAME "clangcl")
		set(TORCH_COMPILER_CLANGCL TRUE)
		...
		set(TORCH_COMPILER_VERSION ${CLANG_MAJOR}${CLANG_MINOR})
		if(TORCH_COMPILER_VERSION LESS "90")
		...
		SET(TORCH_COMPILER_NAME "vc")
		SET(TORCH_COMPILER_MSVC TRUE)
		if(MSVC_VERSION GREATER_EQUAL 1930)
			set(TORCH_COMPILER_VERSION "143")
		elseif(MSVC_VERSION GREATER_EQUAL 1920)
			SET(TORCH_COMPILER_VERSION "142")
		elseif(MSVC_VERSION GREATER_EQUAL 1911)
			SET(TORCH_COMPILER_VERSION "141")
		else()
		...
		IF(TORCH_PLATFORM_WINDOWS_STORE OR (TORCH_ARCH_NAME STREQUAL "arm64"))
		...
		IF((MSVC_VERSION GREATER 1912) AND (TORCH_ARCH_NAME STREQUAL "x64"))
		...
		IF(TORCH_PLATFORM_WINDOWS_STORE)
			FOREACH(flag_var
	...
	set(CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS_RELEASE} /DTORCH_SHIP")
	foreach(flag_var
		CMAKE_CXX_FLAGS_RELEASE CMAKE_CXX_FLAGS_RELWITHDEBINFO CMAKE_CXX_FLAGS_MINSIZEREL)
		set(${flag_var} "${${flag_var}} /fp:fast /Ob2 /GL /Qpar")
	endforeach()
	IF(TORCH_ARCH_NAME MATCHES "x86")
		FOREACH(flag_var
			CMAKE_CXX_FLAGS_RELEASE CMAKE_CXX_FLAGS_RELWITHDEBINFO CMAKE_CXX_FLAGS_MINSIZEREL)
	...
	IF(TORCH_ARCH_NAME MATCHES "arm")
...
ELSE()
	IF(CMAKE_C_COMPILER_ID MATCHES Clang)
		SET(TORCH_COMPILER_NAME "clang")
		SET(TORCH_COMPILER_CLANG TRUE)
	ELSEIF(MINGW)
		SET(TORCH_COMPILER_NAME "mgw")
		SET(TORCH_COMPILER_GCC TRUE)
	ELSE()
		SET(TORCH_COMPILER_NAME "gcc")
		SET(TORCH_COMPILER_GCC TRUE)
	ENDIF()
	...
	IF(TORCH_COMPILER_CLANG)
		EXECUTE_PROCESS(COMMAND ${CMAKE_C_COMPILER} --version OUTPUT_VARIABLE CLANG_VERSION)
		STRING(REGEX MATCHALL "[0-9]+" CLANG_VERSION_COMPONENTS ${CLANG_VERSION})
		LIST(GET CLANG_VERSION_COMPONENTS 0 CLANG_MAJOR)
		LIST(GET CLANG_VERSION_COMPONENTS 1 CLANG_MINOR)
		SET(TORCH_COMPILER_VERSION ${CLANG_MAJOR}${CLANG_MINOR})
		if(TORCH_PLATFORM_DARWIN OR TORCH_PLATFORM_IOS)
			if(TORCH_COMPILER_VERSION LESS "61")
				message(FATAL_ERROR "Unsupported compiler version. Please install Apple clang++ 6.1 or up.")
			endif()
		elseif(TORCH_PLATFORM_ANDROID)
			if(TORCH_COMPILER_VERSION LESS "50")
				message(FATAL_ERROR "Unsupported compiler version. Please install clang++ 5.0 (NDK 16) or up.")
			endif()
		elseif(TORCH_PLATFORM_LINUX)
			if(TORCH_COMPILER_VERSION LESS "100")
	...
	ELSE()
		...
		SET(TORCH_COMPILER_VERSION ${GCC_MAJOR}${GCC_MINOR})
		if(TORCH_COMPILER_VERSION LESS "90")
	...
	FOREACH(flag_var
		...
		ENDIF()
		IF(TORCH_COMPILER_CLANG AND (TORCH_PLATFORM_DARWIN OR TORCH_PLATFORM_IOS))
	...
	IF(TORCH_ARCH_NAME STREQUAL "x86")
		...
		ENDIF()
	ELSEIF((TORCH_ARCH_NAME STREQUAL "x64") OR (TORCH_ARCH_NAME STREQUAL "x86_64"))
		FOREACH(flag_var
			CMAKE_C_FLAGS CMAKE_CXX_FLAGS)
		...
		ENDIF()
	ENDIF()
	IF(NOT TORCH_HOST_PLATFORM_DARWIN)
		FOREACH(flag_var
...
SET(TORCH_OUTPUT_SUFFIX _${TORCH_COMPILER_NAME}${TORCH_COMPILER_VERSION})
...
IF(MSVC)
	...
	FUNCTION(CREATE_VCPROJ_USERFILE TARGETNAME)
		...
		CONFIGURE_FILE(
			${TORCH_CMAKE_MODULE_DIR}/VisualStudio2010UserFile.vcxproj.user.in
			${CMAKE_CURRENT_BINARY_DIR}/${TARGETNAME}.vcxproj.user
			@ONLY
		)
	ENDFUNCTION()
ELSEIF(TORCH_PLATFORM_DARWIN OR TORCH_PLATFORM_IOS)
	...
	FUNCTION(CREATE_XCODE_USERFILE PROJECTNAME TARGETNAME)
		IF(TORCH_PLATFORM_DARWIN OR TORCH_PLATFORM_IOS)
			...
			CONFIGURE_FILE(
				${TORCH_CMAKE_MODULE_DIR}/xcode.xcscheme.in
...
FUNCTION(TORCH_ADD_PRECOMPILED_HEADER TARGET_NAME PRECOMPILED_HEADER)
	IF(TORCH_COMPILER_MSVC OR TORCH_COMPILER_CLANGCL)
		IF(CMAKE_GENERATOR MATCHES "^Visual Studio")
			TORCH_ADD_MSVC_PRECOMPILED_HEADER(${TARGET_NAME} ${PRECOMPILED_HEADER})
		ENDIF()
	ELSEIF(TORCH_PLATFORM_DARWIN OR TORCH_PLATFORM_IOS)
		TORCH_ADD_XCODE_PRECOMPILED_HEADER(${TARGET_NAME} ${PRECOMPILED_HEADER})
	ELSE()
		TORCH_ADD_GCC_PRECOMPILED_HEADER(${TARGET_NAME} ${PRECOMPILED_HEADER})
	ENDIF()
ENDFUNCTION()

FUNCTION(TORCH_ADD_MSVC_PRECOMPILED_HEADER TARGET_NAME PRECOMPILED_HEADER)
...
FUNCTION(TORCH_ADD_GCC_PRECOMPILED_HEADER TARGET_NAME PRECOMPILED_HEADER)
...
FUNCTION(TORCH_ADD_XCODE_PRECOMPILED_HEADER TARGET_NAME PRECOMPILED_HEADER)
