...
template <uint64_t ch0, uint64_t ch1, uint64_t ch2,
    ...
    template <int c>
	inline ElementChannel Channel(ElementFormat ef)
	{
		return static_cast<ElementChannel>((static_cast<uint64_t>(ef) >> (4 * c)) & 0xF);
	}

	template <int c>
	inline ElementFormat Channel(ElementFormat ef, ElementChannel new_c)
	{
	...
	template <int c>
	inline uint8_t ChannelBits(ElementFormat ef)
	{
		return (static_cast<uint64_t>(ef) >> (16 + 6 * c)) & 0x3F;
	}

	template <int c>
	inline ElementFormat ChannelBits(ElementFormat ef, uint64_t new_c)
	{
		uint64_t ef64 = static_cast<uint64_t>(ef);
		ef64 &= ~(0x3FULL << (16 + 6 * c));
		ef64 |= (new_c << (16 + 6 * c));
	...
	template <int c>
    inline ElementFormat ChannelType(ElementFormat ef, ElementChannelType new_c)
	{
		uint64_t ef64 = static_cast<uint64_t>(ef);
		ef64 &= ~(0xFULL << (40 + 4 * c));
	...
	inline bool IsSigned(ElementFormat format)
	{
		return (ECT_SNorm == ChannelType<0>(format));
	}

	inline uint8_t NumFormatBits(ElementFormat format)
	{
		switch (format)
		{
		case EF_BC1:
    ...
    inline uint8_t NumFormatBytes(ElementFormat format)
	{
    ...
    inline ElementFormat MakeNonSRGB(ElementFormat format)
	{
    ...
    inline ElementFormat MakeSigned(ElementFormat format)
	{
		if (ECT_UNorm == ChannelType<0>(format))
		{
	...
	inline ElementFormat MakeUnsigned(ElementFormat format)
	{
	...
	inline uint32_t NumComponents(ElementFormat format)
	{
		switch (format)
		{
		case EF_BC4:
	...
	{
		return glm::max(glm::max(ChannelBits<0>(format), ChannelBits<1>(format)),
			glm::max(ChannelBits<2>(format), ChannelBits<3>(format)));

	}

	// TORCH_CORE_API void ConvertToABGR32F(ElementFormat fmt, void const * input, uint32_t num_elems, Color* output);
	// TORCH_CORE_API void ConvertFromABGR32F(ElementFormat fmt, Color const * input, uint32_t num_elems, void* output);
